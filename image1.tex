...
また、上記処理を以下の組み合わせで行う。
\begin{itemize}
	\item $k=7$、$\sigma=4$、画像kodak001。
	\item $k=7$、$\sigma=2$、画像kodak001。
	\item $k=7$、$\sigma=1$、画像kodak001。
\end{itemize}
...
次に、$k=3$,$\sigma=1$と$k=15$,$\sigma=8$におけるガウシアンフィルタの周波数振幅特性のグラフ$G$と入力画像$\mathbf{X}$、出力画像$\mathbf{Y}$をそれぞれ示す。
...
\begin{figure}[H]
	...
	\begin{tabular}{ccc}
		...
		%%%%%%%%%
	\end{tabular}
	\caption{ガウシアンフィルタの周波数振幅特性と画像に適用した場合の入力と出力画像($k=3$と$\sigma=1$)}
	\label{fig:2-1-2}
\end{figure}
%%%%%%%%%%%%%%%%%%

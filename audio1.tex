...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{ファイル_000.png}
    \caption{Sub3-3-1.adのスペクトル包絡}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{ファイル_001.png}
    \caption{Sub3-3-2.adのスペクトル包絡}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{ファイル_002.png}
...
Sub3-3-2 は，
/u/ と/o/に比較的近い位置に存在している。Sub3-3-1の方を/o/と判断したため
...
これは，
ナイキスト周波数を超える成分は正しく表現できず，
折り返し雑音として観測されるためである。

したがって，
今回の実験において解析できる周波数範囲は
$0\sim8000\,\mathrm{Hz}$ である。
...
ハニング窓は，
両端が0になるように滑らかに変化する窓である。
信号の端での不連続を小さくできるため，
矩形窓よりもスペクトル漏れを抑えやすい。

ブラックマン窓は，
ハニング窓よりもさらに端の影響を小さくする窓である。
不要な周波数成分をより抑えられるが，
主なピークの幅が広がりやすい。

このように，
窓関数は信号の切り出し区間の端で生じる不連続を緩和し，
DFTの結果に現れる不要な周波数成分の広がりを抑えるために用いられる。

\subsection*{(WH4)}

フォルマントとは，
音声スペクトルにおいて強く現れる共振周波数のことである。
特に母音では，
第1フォルマント周波数 $F_1$ と
第2フォルマント周波数 $F_2$ が，
母音の違いを表す重要な特徴となる。
...
今回の実験では，
未知音声データの対数パワースペクトルを求め，
スペクトル包絡から $F_1$ と $F_2$ を読み取った。
さらに，
得られたフォルマント周波数対を
日本語5母音の典型的なフォルマント分布と比較した。

その結果，
Sub3-3-1 は /o/，
Sub3-3-2 は /u/，
Sub3-3-3 は /e/ に近い位置に分布していた。
このことから，
フォルマント周波数を用いることで，
未知音声データの母音を推定できることが確認できた。
\section{感想}
